...
test("multi-wallet discovery keeps provider identity and rejects conflicting labels", async () => {
  const calls = [];
  const okx = { request: async () => { calls.push("okx"); return ["0x1111111111111111111111111111111111111111"]; } };
  const sharedProxy = { isOkxWallet: true, isPhantom: true, request: async () => { calls.push("shared-proxy"); return []; } };
  const metamask = { isMetaMask: true, request: async () => { calls.push("metamask"); return ["0x2222222222222222222222222222222222222222"]; } };
  const win = new FakeWindow({ providers: [metamask, sharedProxy] }, [
    { info: { uuid: "okx", rdns: "com.okex.wallet", name: "OKX Wallet" }, provider: sharedProxy },
  ...
  assert.deepEqual(calls, ["okx"]);
  result.stop();
});

test("OKX is omitted when its direct namespace is unavailable", () => {
  const proxy = { isOkxWallet: true, request: async () => [] };
  const win = new FakeWindow(undefined, [
    { info: { uuid: "okx", rdns: "com.okex.wallet", name: "OKX Wallet" }, provider: proxy },
  ]);
  const result = collectProviders(win);
