...
test("multi-wallet discovery keeps provider identity and rejects conflicting labels", async () => {
  const calls = [];
  const okx = { isOkxWallet: true, request: async () => { calls.push("okx"); return ["0x1111111111111111111111111111111111111111"]; } };
  const metamask = { isMetaMask: true, request: async () => { calls.push("metamask"); return ["0x2222222222222222222222222222222222222222"]; } };
  const win = new FakeWindow({ providers: [metamask, okx] }, [
    { info: { uuid: "okx", rdns: "com.okex.wallet", name: "OKX Wallet" }, provider: okx },
